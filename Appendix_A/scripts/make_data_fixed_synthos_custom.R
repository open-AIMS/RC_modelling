...
simulated_field_sf  <- synthos:::generate_field(spatial.grid, config_sp)
...
mesh <- synthos:::create_spde_mesh(spatial.grid, config_sp)
spde <- synthos:::create_spde(spatial.grid, config_sp)

######### DHW 
# pts effects 
dhw.pts.effects.df <- synthos:::disturbance_dhw(spatial.grid,spde,config_sp)$dhw_pts_effects_df %>%
                      mutate(Dist = "DHW") %>%
...
# effects
cyc.effects <- synthos:::disturbance_cyc(spatial.grid,spde,config_sp)$cyc_effects

## Vizualisation 
########## All
all.pts.effects <- bind_rows(dhw.pts.effects.df, cyc.pts.effects) %>%
...
if (length(unique(all.pts.effects$Year)) < 9) {
  ncols <- 3
} else if (length(unique(all.pts.effects$Year)) >= 20) {
  ncols <- 5
} else {
  ncols <- 4
}
...
plots_by_dist <- map2(
  ...
  ~ ggplot() +  
      ...
      scale_fill_viridis_c(
        ...
        direction = -1,
        na.value = "grey90",
        limits = c(0, ceiling(max(.x$Value_weighted) * 100) / 100),
        breaks = seq(0, ceiling(max(.x$Value_weighted) * 100) / 100, by = 0.1),
...
n_panels <- length(unique(all.pts.effects$Year))

base_width  <- 4
base_height <- 3

plot_width  <- ncols * base_width
plot_height <- ceiling(n_panels / ncols) * base_height
...
ggsave(filename = paste0(title_of_run,"/report/extra/cyclone.png"),
       plot = plots_by_dist[[1]], width= plot_width, height= plot_height)

ggsave(filename = paste0(title_of_run,"/report/extra/dhw.png"),
       plot = plots_by_dist[[2]], width= plot_width, height= plot_height)
...
baseline.sample.hcc <- mesh$loc[,1:2] %>%
  ...
  mutate(clong = as.vector(scale(Longitude, scale = FALSE)),
         ...
         Y = clong + sin(clat) + #rnorm(1,0,1) +
           1.5*clong + clat) %>%
  mutate(Y = scales::rescale(Y, to = qlogis(config_sp$hcc_cover_range)))
...
baseline.sample.sc <- mesh$loc[,1:2] %>%
  ...
  mutate(clong = as.vector(scale(Longitude, scale = FALSE)),
         ...
         Y = clong + sin(clat) + #rnorm(1,0,1) +
           1.5*clong + -1.5*clat) %>%
  mutate(Y = scales::rescale(Y, to = qlogis(config_sp$sc_cover_range)))
...
reef_data.synthetic_fixed_ready <- reef_data.synthetic_fixed_ready %>%
  dplyr::select(P_CODE:LONGITUDE,fYEAR,fDEPTH, fGROUP, TRUE_COUNT,TOTAL, COUNT)
