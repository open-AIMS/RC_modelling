...
nth_element <- function(vector, starting_position = 1, n = 1) {
  if (length(vector) < n) {
    return(vector)
  } else {
    return(vector[seq(starting_position, length(vector), n)])
  }
}
##################################
############ Extract covariates 
...
p_traj <- ggplot() + 
 ...
 geom_line(data = pred_df %>% filter(tier %in% unique_tier), 
  ...
  scale_x_continuous(
    breaks = seq(
      floor(min(as.numeric(pred_df$fYEAR))), 
      ceiling(max(as.numeric(pred_df$fYEAR))), 
      by = 2  
    )
  )
