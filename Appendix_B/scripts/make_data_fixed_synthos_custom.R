...
if (length(unique(dhw.pts.effects.df$Year)) < 9) {
  ncols <- 3
} else if (length(unique(dhw.pts.effects.df$Year)) > 20) {
  ncols <- 5
} else {
  ncols <- 4
}
...
########## OT
# pts effects
other.pts.effects <- synthos:::disturbance_other(spatial.grid,spde,config_sp)$other_pts_effects
...
# effects
other.effects <- synthos:::disturbance_other(spatial.grid,spde,config_sp)$other_effects
